...
// fn get_process_data(system: &System) -> Vec<(i32, f32, f64, String, sysinfo::ProcessStatus)> {
//     let mut rows = vec![];
...
//     loop {
//         system.refresh_processes();
//         let rows = get_process_data(&system);
...
fn main() {
    ...
    for (pid, proc_) in system.get_processes() {
        println!("{}:{} => status: {:?}, cpu_usage: {}", pid, proc_.name(), proc_.status(), proc_.cpu_usage());
    }
